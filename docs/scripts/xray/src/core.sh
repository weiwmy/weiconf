...
change() {
    ...
    11)
        ...
        [[ $(grep -i "^weiwmy.net$" <<<$is_servername) ]] && {
            err "你干嘛～哎呦～"
        }
        add $net
        ;;
    ...
    if [[ $is_use_tls ]]; then
        if [[ ! $is_no_auto_tls && ! $is_caddy && ! $is_gen ]]; then
            ...
            [[ $(is_test port_used 80) || $(is_test port_used 443) ]] && {
                ...
                msg "\e[41m帮助(help)\e[0m: $(msg_ul https://cf.weiwmy.net/$is_core/no-auto-tls/)\n"
                exit 1
            }
            is_install_caddy=1
        fi
...
get() {
    ...
    protocol)
        ...
        *socks*)
            is_protocol=socks
            net=socks
            [[ ! $is_socks_user ]] && is_socks_user=weiwmy
...
info() {
    ...
    tcp | kcp | quic)
        ...
        is_vmess_url=$(jq -c '{v:2,ps:'\"weiwmy-${net}-$is_addr\"',add:'\"$is_addr\"',port:'\"$port\"',id:'\"$uuid\"',aid:"0",net:'\"$net\"',type:'\"$header_type\"',path:'\"$kcp_seed\"'}' <<<{})
    ...
    ss)
        ...
        is_url="ss://$(echo -n ${ss_method}:${ss_password} | base64 -w 0)@${is_addr}:${port}#weiwmy-$net-${is_addr}"
    ...
    ws | h2 | grpc)
        ...
        [[ $is_protocol == 'vmess' ]] && {
            is_vmess_url=$(jq -c '{v:2,ps:'\"weiwmy-$net-$host\"',add:'\"$is_addr\"',port:'\"$tlsport\"',id:'\"$uuid\"',aid:"0",net:'\"$net\"',host:'\"$host\"',path:'\"$path\"',tls:'\"tls\"'}' <<<{})
            is_url=vmess://$(echo -n $is_vmess_url | base64 -w 0)
        } || {
            [[ $is_trojan ]] && {
            ...
            is_url="$is_protocol://$uuid@$host:$tlsport?encryption=none&security=tls&type=$net&host=$host&${is_url_path}=$(sed 's#/#%2F#g' <<<$path)#weiwmy-$net-$host"
        }
    ...
    reality)
        ...
        is_url="$is_protocol://$uuid@$ip:$port?encryption=none&security=reality&flow=xtls-rprx-vision&type=tcp&sni=$is_servername&pbk=$is_public_key&fp=ios#weiwmy-$net-$is_addr"
        ;;
    door)
    ...
    socks)
        ...
        is_url="socks://$(echo -n ${is_socks_user}:${is_socks_pass} | base64 -w 0)@${is_addr}:${port}#weiwmy-$net-${is_addr}"
        ;;
    http)
        is_can_change=(0 1)
        is_info_show=(0 1 2)
...
url_qr() {
    ...
    if [[ $is_url ]]; then
        [[ $1 == 'url' ]] && {
            ...
            link="https://cf.weiwmy.net/tools/qr.html#${is_url}"
